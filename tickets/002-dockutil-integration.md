...
- Resolving path: `dockutil` found on PATH first, else the config'd path from 006; run detection via `dockutil --version` (prints a bare version string, confirmed `print(VERSION)` → e.g. `3.1.3`).
...
Decisions:
- **Shell out vs. embed**: Shell out via `Process`, always as the **invoking user** (never sudo/root — avoids the open defaults-hang family dockutil#126/#160/#153). Binary path resolved from config 006 with PATH fallback.
